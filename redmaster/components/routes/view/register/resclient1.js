...
const FormClient = () => {
  ...
  const handleRadioButtonStatic = (value) => {
    setInputs({ ...inputs, class: value });
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View>
          <RadioButtonGroup
          containerStyle={{ marginBottom: 10,with:"100%", flexDirection: "row", gap: 25 }}
          selected={inputs.class}
          onSelected={(value) => handleRadioButtonStatic(value)}
          radioBackground="#242424"
           >
          <RadioButtonItem
            value="solteiro"
            label={<Text style={{ color: "#ffffff", fontSize: 15 }}>Solteiro(a)</Text>}
          />
          <RadioButtonItem
            style={{ marginLeft:15}}
            value="casado"
            label={<Text style={{ color: "#ffffff", fontSize: 15 ,}}>Casado(a)</Text>}
          />
          </RadioButtonGroup>
           <RadioButtonGroup
          containerStyle={{ marginBottom: 10,with:"100%", flexDirection: "row", gap: 20 }}
          selected={inputs.estado}
          onSelected={(value) => handleRadioButtonStatic(value)}
          radioBackground="#242424"
             >
          <RadioButtonItem
            value="divorciado"
            label={<Text style={{ color: "#ffffff", fontSize: 15 }}>Divorciado(a)</Text>}
          />
          <RadioButtonItem
            value="separado"
            label={<Text style={{ color: "#ffffff", fontSize: 15 }}>Separado(a)</Text>}
          />
           </RadioButtonGroup>
        </View>
